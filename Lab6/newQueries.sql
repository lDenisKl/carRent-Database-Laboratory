-- 1. Рассчитать выручку пункта проката по датам с начала текущего месяца
SELECT 
...
-- 2. Для каждого типа и модели автомобиля вывести количество машин
SELECT 
    m.type AS car_type,
...
-- 3. Найти модели, не пользующиеся спросом (с начала текущего года)
SELECT 
    m.id,
    m.name,
    m.type,
...
WHERE m.id NOT IN (
    ...
    WHERE MATCH(ca-(om)->m2 AND o-(h)->ca)
        ...
        AND h.rentalStatus IN ('Активна', 'Завершена')
);
...
-- 4. Найти постоянных клиентов (более 3-х раз) с расчётом скидки
WITH ClientRentals AS (
    ...
    WHERE MATCH(c-(po)->o-(h)->ca)
        AND h.rentalStatus IN ('Завершена', 'Активна')
...
-- 5. Найти клиентов, наиболее часто пользующихся услугами проката
SELECT TOP 10
    c.passport,
    c.fullName,
    c.phone,
